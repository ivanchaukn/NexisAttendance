...
public class Data {
    ...
    public static ArrayList<Integer> getRecentFellowshipData (List<ParseObject> nexcellObject, DateTime date)
    {
        ...
        for(int row = nexcellObject.size() - 1; date.equals(rowDate); row--)
        {
            ...
            {
                if (Constants.NEXCELL_ACTIVE_LIST.get(i).equals(rowNexcell))
                {
    ...
    public static ArrayList<Integer> getAverageData (List<ParseObject> nexcellObject, String type)
    {
        ...
        for(int row = nexcellObject.size() - 1; row >= 0; row--)
        {
            ...
            if (Constants.NEXCELL_ACTIVE_LIST.contains(nexcellObject.get(row).get("Nexcell")))
            {
                int pos = Constants.NEXCELL_ACTIVE_LIST.indexOf(rowNexcell);
